    ...
    <div className="h-full flex flex-col">
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-[minmax(3.5rem,1fr)] md:auto-rows-[minmax(8rem,1fr)] p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          ...
          const isPast = isBefore(_date, startOfDay(new Date()));

          return (
            <div
              className={cn(
                'ring-1 p-2 text-sm text-muted-foreground ring-border overflow-auto cursor-pointer hover:bg-accent/50 transition-colors',
                !isSameMonth(date, _date) && 'text-muted-foreground/50',
                isPast && 'opacity-50 pointer-events-none bg-muted/20'
              )}
              key={_date.toString()}
              onClick={() => !isPast && onDayClick?.(_date)}
            >
              <span
                className={cn(
