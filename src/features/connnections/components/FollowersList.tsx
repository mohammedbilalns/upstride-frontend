...
export default function FollowersList({ searchQuery, sortBy }: FollowersListProps) {
	...
	const sortConnections = (connections: any[]) => {
		...
		switch (sortBy) {
			case "newest":
				return sorted.sort((a, b) => new Date(b.connectedAt || 0).getTime() - new Date(a.connectedAt || 0).getTime());
			case "oldest":
				return sorted.sort((a, b) => new Date(a.connectedAt || 0).getTime() - new Date(b.connectedAt || 0).getTime());
			case "name":
								...
								<div key={follower.id} className="flex items-center justify-between p-4 border rounded-lg">
									<div className="flex items-center">
										<div className="relative">
											<UserAvatar image={follower.user.profilePicture} name={follower.user.name} size={12} />
											{follower.isOnline && (
										...
										<div className="ml-4">
											...
											<p className="text-sm text-muted-foreground">{follower.expertise.name}</p>
											<p className="text-xs text-muted-foreground mt-1">
												Following since {new Date(follower.connectedAt).toLocaleDateString()}
											</p>
										</div>
									</div>
