...
export default function FollowedList({ searchQuery, sortBy }: FollowedListProps) {
		const {
		data,
		fetchNextPage,
		hasNextPage,
	...
	const filteredFollowing = mentors.filter(mentor =>
								...
								{sortedFollowing.map((mentor) => (
									<div key={mentor.id} className="flex items-center justify-between p-4 border rounded-lg">
										<div className="flex items-center">
											<UserAvatar image={mentor.user?.profilePicture} name={mentor.user?.name} size={12} />
											<div className="ml-4">
												<h3 className="font-medium">{mentor.user?.name}</h3>
												<p className="text-sm text-muted-foreground">{mentor.expertise?.name}</p>
