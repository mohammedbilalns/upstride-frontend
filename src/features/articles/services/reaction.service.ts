...
export function reactResource(resourceId: string, reaction: "like" | "dislike", resourceType: "article" | "comment") {
	return apiRequest(() => api.post(API_ROUTES.REACT.REACT_RESOURCE, {
		resourceId,
		reaction,
		resourceType,
	}))
...
export function getReactions(resourceId: string, page: number, limit: number) {
	return apiRequest(() => api.get(API_ROUTES.REACT.GET_REACTIONS, {
		params: { resourceId, page, limit },
	}))
}
